...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    if [ ! -z ${asr_inference_text} ]; then 
        # Replace original transcript with predicted transcript from ASR model
        cp ${asr_inference_text} data/test_${data_tag}/text
        cp ${asr_inference_text} data/test_${data_tag}/text.${src_lang}

        python3 local/remove_blank_lines.py \
            --src_language ${src_lang} \
            --tgt_language ${tgt_lang} \
            --data_folder data/test_${data_tag}
    fi
